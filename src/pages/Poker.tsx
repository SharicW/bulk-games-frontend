...
function Poker() {
  ...
  const { bestHand, best5Keys, kickerKeysSet } = useMemo(() => {
    if (!rawBestHand || !gameState) {
      trackedRef.current = null
      return { bestHand: null as HandResult | null, best5Keys: new Set<string>(), kickerKeysSet: new Set<string>() }
    }

    const prev = trackedRef.current
    const newKeys = new Set(rawBestHand.best5.map(c => cardKey(c)))
    const newKickerKeys = new Set(rawBestHand.kickerKeys)
    ...
    if (!prev || prev.handNumber !== gameState.handNumber || rawBestHand.score > prev.score) {
      ...
      return { bestHand: rawBestHand, best5Keys: newKeys, kickerKeysSet: newKickerKeys }
    }
            ...
            <div className="poker-table__felt">
              ...
              <div className="poker-table__community">
                {gameState.communityCards.map((card) => {
                  ...
                  return (
                    <CardDisplay key={k} card={card} highlighted={hl} dimmed={bestHand !== null && !hl} isKicker={hl && isK} />
                  )
                })}
