...
import { getBestHand } from '../utils/handEval'
...
function Poker() {
  ...
  // Index-based highlight sets — no string matching, purely positional.
  // holeHighlight:  Set of indices (0 or 1) into myHoleCards that are in the best 5
  // commHighlight:  Set of indices (0–4) into communityCards that are in the best 5
  const holeHighlight = new Set<number>(bestHand?.holeIndices ?? [])
  const commHighlight = new Set<number>(bestHand?.communityIndices ?? [])
        ...
        <div className="poker-table-wrapper">
          <div className="poker-table">
            <div className="poker-table__felt">
              ...
              {/* Community cards — highlight only if index is in the best 5 */}
              <div className="poker-table__community">
                {gameState.communityCards.map((card, idx) => (
                  <CardDisplay key={`${card.rank}_${card.suit}`} card={card} highlighted={commHighlight.has(idx)} dimmed={bestHand !== null && !commHighlight.has(idx)} />
                ))}
                {/* Empty placeholders */}
        ...
        <div className="poker-bottom-bar">
          {gameState.myHoleCards.length > 0 && (
            <div className="poker-my-cards">
              <CardDisplay card={gameState.myHoleCards[0]} highlighted={holeHighlight.has(0)} dimmed={bestHand !== null && !holeHighlight.has(0)} />
              <CardDisplay card={gameState.myHoleCards[1]} highlighted={holeHighlight.has(1)} dimmed={bestHand !== null && !holeHighlight.has(1)} />
            </div>
          )}
          {bestHand && (
