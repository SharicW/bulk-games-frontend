...
const CardDisplay = memo(function CardDisplay({ card, isHidden = false, highlighted = false, dimmed = false }: { card: Card | null; isHidden?: boolean; highlighted?: boolean; dimmed?: boolean }) {
  if (!card || isHidden) {
    return (
  ...
  return (
    <div className="poker-card" style={cardStyle}>
      {imageUrl ? (
        <img src={imageUrl} alt={formatCard(card)} className="poker-card__image" decoding="async" />
      ) : (
...
function Poker() {
  ...
  const rawBestHand = useMemo<HandResult | null>(() => {
  ...
  }, [gameState?.gameStarted, gameState?.myHoleCards, gameState?.communityCards, me?.folded])

  const trackedRef = useRef<{ handNumber: number; score: number; keys: Set<string>; name: string } | null>(null)

  const { bestHand, best5Keys } = useMemo(() => {
    if (!rawBestHand || !gameState) {
      trackedRef.current = null
      return { bestHand: null as HandResult | null, best5Keys: new Set<string>() }
    }

    const prev = trackedRef.current
    const newKeys = new Set(rawBestHand.best5.map(c => cardKey(c)))

    if (!prev || prev.handNumber !== gameState.handNumber || rawBestHand.score > prev.score) {
      trackedRef.current = {
        handNumber: gameState.handNumber,
        score: rawBestHand.score,
        keys: newKeys,
        name: rawBestHand.name,
      }
      return { bestHand: rawBestHand, best5Keys: newKeys }
    }

    // Keep previous (equal or better) — return stored data
    return {
      bestHand: { ...rawBestHand, name: prev.name, best5: rawBestHand.best5 } as HandResult,
      best5Keys: prev.keys,
    }
  }, [rawBestHand, gameState?.handNumber])
