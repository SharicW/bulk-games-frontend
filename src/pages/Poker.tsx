...
function Poker() {
  ...
  // Build a highlight Set DIRECTLY from the evaluator's exact 5-card selection.
  // No fallback logic — if the set isn't exactly 5 unique keys, highlight nothing.
  const highlightSet = new Set<string>()
  if (bestHand && bestHand.best5Keys.length === 5) {
    for (const k of bestHand.best5Keys) highlightSet.add(k)
    // Final guard: must be exactly 5 unique keys
    if (highlightSet.size !== 5) highlightSet.clear()
  }
  const isHighlighted = (card: Card): boolean => highlightSet.has(cardKey(card))
            ...
            <div className="poker-table__felt">
              ...
              <div className="poker-table__community">
                {gameState.communityCards.map((card) => (
                  <CardDisplay key={`${card.rank}_${card.suit}`} card={card} highlighted={isHighlighted(card)} />
                ))}
                {/* Empty placeholders */}
        ...
        <div className="poker-bottom-bar">
          {gameState.myHoleCards.length > 0 && (
            <div className="poker-my-cards">
              <CardDisplay card={gameState.myHoleCards[0]} highlighted={isHighlighted(gameState.myHoleCards[0])} />
              <CardDisplay card={gameState.myHoleCards[1]} highlighted={isHighlighted(gameState.myHoleCards[1])} />
            </div>
          )}
          {bestHand && (
