...
import { useUnoSync } from '../hooks/useUnoDesktopSync'
...
export default function UnoRouter() {
  ...
  const lobbyCode = (searchParams.get('lobby') || searchParams.get('code') || '').toUpperCase()
  ...
  const sync = useUnoSync(lobbyCode, isLoggedIn, user?.id)
  return <UnoUI lobbyCode={lobbyCode} isLoggedIn={isLoggedIn} userId={user?.id} sync={sync} isMobile={isMobile} />
}
...
function UnoUI({ lobbyCode, isLoggedIn, userId, sync, isMobile }: {
  ...
  sync: ReturnType<typeof useUnoSync>
  isMobile: boolean
}) {
