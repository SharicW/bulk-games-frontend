...
const UnoCardImg = memo(function UnoCardImg({ card, images, className, dimmed, glow, onClick }: {
  ...
  return (
    ...
    >
      {src ? (
        <img src={src} alt={cardLabel(card.face)} decoding="async" />
      ) : (
        ...
        <div className="uno-bottom-bar">
          <div className="uno-actions">
            ...
            <div className="uno-actions__buttons">
              ...
              {/* UNO! — shown when I must call UNO (I have 1 card left) */}
              {state.mustCallUno === uid && (
                <button
                  className="btn-primary uno-actions__btn uno-actions__btn--uno"
                  onClick={() => unoSocket.callUno(state.lobbyCode)}
                >
                  UNO!
                </button>
              )}
              {/* Catch — shown when someone ELSE must call UNO */}
              {state.mustCallUno && state.mustCallUno !== uid && (
                <button
                  className="btn-primary uno-actions__btn uno-actions__btn--catch"
                  onClick={() => unoSocket.catchUno(state.lobbyCode)}
                >
                  Catch!
                </button>
              )}
