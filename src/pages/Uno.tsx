...
function Uno() {
  ...
  useEffect(() => {
    ...
    const join = async () => {
      const result = await unoSocket.joinLobby(lobbyCode, uid, profile.nickname, profile.avatarUrl)
      if (stopped) return
  ...
  const sendAction = useCallback(async (action: { type: 'play'; cardId: string; chosenColor?: UnoColor } | { type: 'draw' } | { type: 'pass' }) => {
    if (!state) return
    const result = await unoSocket.sendAction(state.lobbyCode, userIdRef.current, action)
    if (!result.success) {
  ...
  const handleStartGame = useCallback(async () => {
    if (!state) return
    const result = await unoSocket.startGame(state.lobbyCode, userIdRef.current)
    if (!result.success) {
  ...
  const handleEndLobby = useCallback(async () => {
    if (!state) return
    const result = await unoSocket.endLobby(state.lobbyCode, userIdRef.current)
    if (result.success) {
      window.close()
    } else {
